...
Продуктовый порядок обновлён 2026-09-05: [COURSE_ROADMAP.md](COURSE_ROADMAP.md). Текущий рабочий host — macOS; Linux-пути в [PLAY_STORE_RELEASE.md](PLAY_STORE_RELEASE.md) относятся к отдельному окружению сборки и не заменяют этот setup. SDK в этой итерации не обновлялся. Android — приоритет публикации; iOS отложен по бюджету, но проверки общего runtime выполняются по риску.
