...
| # | Пункт | Статус |
| --- | --- | --- |
| 2.1 | Детерминированные сценарии раздачи | **Сделано**, `blackjack_engine_standard_profile_test.dart` (полный перечень: blackjack против blackjack, surrender, dealer peek, resplit x4, DAS, multiple seats, боты со всеми действиями, push/bust/выплаты) |
| 2.2 | Seeded stress-test на 10 000 раундов | **Сделано**, `blackjack_engine_stress_test.dart` (100 seed × 100 раундов) |

Все сценарии 2.1 полностью сверены и защищены автоматическими тестами.

## 3. Сквозные UI и платформенные сценарии

| # | Пункт | Статус |
| --- | --- | --- |
| 3.1 | Android integration suite (12 маршрутов) | **Сделано**, `integration_test/app_test.dart` |
| 3.2 | Матрица API 36 | **Сделано**, все 12 маршрутов зелёные |
| 3.3 | Матрица API 24 x86_64 | **Сделано**, все 12 маршрутов зелёные |
| 3.4 | Семь размеров экрана | **Сделано**, `layout_test.dart` (en + ru) |
| 3.5 | Text scale 1.0 и 1.3 | **Сделано**, `layout_test.dart` (Learning path, Drill, Progress, Table) |
...
Все 12 реализованы в `integration_test/app_test.dart` и проходят на обоих AVD
(API 36 за 4:12, API 24 за 4:08, 2026-09-03). Прогон:

```sh
flutter test integration_test/app_test.dart -d emulator-5554   # API 36
flutter test integration_test/app_test.dart -d emulator-5556   # API 24
```

Маршруты 1, 4 и 12 сделаны «толстыми»: они проходят сценарий, повторно
вызывают `main()` и проверяют, что состояние поднялось из настоящего
хранилища. Ради этого suite и нужен, остальное уже покрыто на уровне
ViewModel, поэтому маршруты 2, 3, 5–11 намеренно тонкие: дойти, убедиться что
экран достижим, идти дальше.

Четыре особенности реального устройства, которых widget-тесты не видят. Каждая
ломала маршрут, пока не была учтена, и каждую стоит помнить при добавлении
новых маршрутов:

- `TableScreen` принудительно переключает ориентацию через `SystemChrome`.
  Поворот приходит с платформы уже после того, как `pumpAndSettle` вернулся,
  поэтому шапки какое-то время нет и связка «тап и сразу проверка» гоняется с
  ней наперегонки.
- Вход в Drill поднимает модальный интро-диалог, который глотает все
  последующие тапы, включая нижнюю навигацию. Прогон выглядит зависшим, а не
  упавшим.
- `main()` ставит собственные обработчики ошибок, которые съедают исключения.
  Если их не восстановить, фреймворк рапортует про подменённый `onError`
  вместо настоящей причины.
- Стартовый режим стола зависит от уровня опыта: `experienced` открывает
  Practice, все остальные Guided (`table_screen.dart:51`). Маршруты 8 и 9 это
  фиксируют.

Ещё две ловушки в самих тестах: `scrollUntilVisible` без ограничения крутит
список вечно, если цели на экране нет вообще, и это читается как зависание, а
не как провал; а элементы `DropdownButtonFormField` и `PopupMenuButton` не
существуют на экране, пока меню не раскрыто.
...
| 5 | Пункты 3.6, 3.7: lifecycle-сценарии, accessibility (tap targets, Semantics). |
| 6 | Пункт 5.6: чек-лист ручного QA, сборка кандидата, передача техническому тестировщику. |
