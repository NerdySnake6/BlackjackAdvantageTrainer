...
Заморожено на время спринта: новые уроки, IAP, реклама, русская локализация UI,
дополнительные rule profiles.
...
| --- | --- | --- |
| Тестов проходит | 53 | 101 |
| domain | 94,5% | **97,6%** (цель 95%) |
| viewmodels + data + core/persistence | 56,4% | **91,0%** (цель 85%) |
| Всего | 68,2% | **88,2%** (цель 75%) |
...
- Локаль ru-RU: грузится ru-пакет контента, текст в нём английский. Заявленное
  состояние scaffold, не дефект. Дрейф ID между локалями покрыт
...
| 5 | Пункты 3.5, 3.6, 3.7: cross-product размеров и text scale, lifecycle-сценарии, accessibility. |
...
- Спринт тестирует только текущий Android beta-scope.
