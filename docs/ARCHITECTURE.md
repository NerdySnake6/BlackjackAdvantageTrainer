...
- Три пилотных урока загружаются из EN/RU `pilot_lessons.json` в отдельный
  ...
  `PilotLessonViewModel` связывает фиксированный pure-Dart
  `DecisionLessonSession` с `DecisionScene` и `CancellationScene`; маршрут
  `/pilot/:lessonId` доступен из Learn. Это ограниченный пилот, не универсальный
  runtime итерации 12 и не второй движок раздачи.
- Аддитивное поле `ProgressSnapshot.pilotSessions` хранит версию и порядок
  ...
  ответов; две вводные задачи не учитываются.
