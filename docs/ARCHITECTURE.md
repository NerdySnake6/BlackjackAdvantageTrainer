...
- `SessionScorer` вычисляет точность и порог 80% для прохождения текущего урока.
- `MasteryCalculator` отдельно применяет порог 90% к новым задачам.
  `MasteryCheckSession` хранит первые ответы, вариант и подпись банка в
  `ProgressSnapshot.masteryChecks`; завершённая проверка не заменяет `lessonScores`.
...
- Три пилотных урока загружаются из EN/RU `pilot_lessons.json` в отдельный
  `CourseCatalog.pilotLessons`. Шесть MCQ-уроков и банк Quick Review не меняются.
  `PilotLessonViewModel` связывает content-driven pure-Dart
  `DecisionLessonSession` с `DecisionScene` и `CancellationScene`; маршрут
  `/pilot/:lessonId` доступен из Learn. Тип миссии, этап, теория и оценка заданы
  [форматом контента](LESSON_FORMAT.md); сессия поддерживает 2–4 вводных задания
  и десять оцениваемых без отдельной логики на урок. Это не генератор и не
  второй движок раздачи.
- Аддитивное поле `ProgressSnapshot.pilotSessions` хранит версию и порядок
  сценариев, фазу, раскрытые карты, введённый RC, первые ответы, подсказки,
  исправление и номер попытки. Итог и начисленный XP записываются вместе;
  повторное открытие результата не выдаёт награду. Неверный первый ответ не
  заменяется исправлением. Score использует фактические десять оцениваемых
  ответов; вводные задачи не учитываются (в трёх текущих уроках их по две).
  Пять самостоятельных задач урока — повторяемая практика, не checkpoint.
  Отдельный `MasteryCheckViewModel` сохраняет новую проверку без изменения XP,
  streak, review и старого Progress. До конца нет подсказок или обратной связи.
  На урок доступны два непересекающихся варианта по 10 новых задач; после
  неудачи можно перейти только к следующему. Повтор уже раскрытого варианта
  не повышает результат. Неизвестная подпись сохранения не даёт значок проверки.
  Soft 18 пока проверяет перенос на многокарточную руку без Double; это узкий
  checkpoint урока, не сертификация всей стратегии или полной колоды.
- `PilotRewardAdapter` сохраняет прежнюю награду: 10 XP за правильный первый
  ответ и 50 XP за первое прохождение. Флаг `awardedXp` и reward сохраняются
  одной записью; повторная обработка того же результата ничего не начисляет.
  При перезапуске завершённого урока компактный `LessonPerformance` попадает в
  `previousPilotResults`. Learn сравнивает последние две сопоставимые попытки
  по правильным и правильным без подсказок, не используя лучший historical
  score. Другая подпись содержания или повреждённая история не дают сравнения.
- `LessonAdaptation` выбирает дополнительный пример только на границе пяти
  practice и пяти independent задач. Две ошибки в одной наблюдаемой категории
  дают простой разбор; пять правильных без подсказок — пример с дополнительной
  картой. Финальный неверный RC не интерпретируется как доказанная ошибка тега.
  Пример необязательный и неоцениваемый: его ответ/черновик хранятся отдельно
  в pilot session, не меняя first answers, hints, XP, stars или контрольный банк.
- `AdaptiveExampleGenerator` варьирует представления в межблочной практике:
  числовой seed и версия генератора сохраняются в сессии. Новая попытка получает
  новый seed; старые snapshots без seed используют вариант 0 с прежними картами.
  Hard/soft варианты покрывают все dealer upcards, несколько составов руки,
  мягкость нескольких тузов и доступность Double. Hi-Lo включает нейтральные
  карты и положительный/отрицательный/нулевой итог. Генерация конечная и не
  считается новым checkpoint; контрольный банк остаётся отдельным и неизменным.
- `LearnCardView` добавляет масштабирование, контрастный красный цвет и одно
  локализованное semantic-название поверх общего `PlayingCardView`; стандартные
  параметры Table не меняются. `DecisionScene` складывает панели вертикально
  на узком экране/при крупном тексте. Banner сохранения ограничен по высоте,
  чтобы не перекрывать повторное действие. См. `LEARN_ACCESSIBILITY.md`.
- Сессия schema 2 хранит locale-independent `contentSignature` из структуры
  урока. `PilotProgressMigration` при старте переносит совместимую schema 1
  одним сохранением без пересчёта XP и без изменения других полей прогресса.
  Повторный запуск — no-op; при ошибке записи исходный снимок остаётся доступным.
  Неизвестные/повреждённые сессии сохраняются для явного локального перезапуска.
- Интерфейс различает правильный первый ответ и исправленный ответ отдельным
  заголовком. Hi-Lo ввод доступен после раскрытия всех карт; кнопки учитывают
  тот же диапазон `initialCount ± cards.length`, что и доменная сессия.
- Несовместимое сохранение блокирует только текущий пилотный урок и предлагает
  явный перезапуск. Старые sessions/scores/review, XP/streak, язык, experience
  и consent не сбрасываются. Ошибка записи оставляет последнее сохранённое
  состояние; сообщение о повторении действия закреплено вне прокрутки и
  объявляется как accessibility live region. Новые telemetry
  события и передача ответов не добавлены.
...
Итерация 21 (2026-09-08): после разрешения владельца временно отложить пилот
перед расширением Free четыре старых `/lesson/:lessonId` открывают новый
игровой контент. `CourseCatalog.foundationLessons` загружается отдельно из
`foundation_lessons.json`; `playableLessons` объединяет его с тремя пилотами
для существующего runtime и сохранения. `pilotLessons` остаётся банком трёх
пилотов и не расширяет область `MasteryCheckBank` автоматически.

`FoundationScene` показывает сборку руки и ввод суммы/типа/состояния;
`actionMeaning` использует `DecisionScene` и фиксированный показ последствия.
Это обучение смыслу действия, не стратегическая рекомендация или новый движок
раздачи. `HandEvaluator` переиспользуется, последствия сверены с BlackjackEngine.
Ранние ответы заблокированы до раскрытия всех карт, сумма скрыта на независимых
задачах. Те же первые ответы, correction, сохранение, 80% и XP-адаптер действуют
без новых видов прогресса. Сигнатура новых миссий включает afterSplit/drawCards;
сигнатуры трёх пилотов не меняются.

MCQ-каталог и stable exercise IDs сохранены для Quick Review. Старый
`activeSessions` не стирается: кнопка на теории ведёт к `/legacy-lesson/:lessonId`
для завершения предыдущего формата. Лучший score, XP, streak, consent и review
не сбрасываются; ранее завершённый урок не получает второй first-completion
бонус. Опытный игрок по-прежнему может начать с `first-strategy`.

Итерация 22: `CourseCatalog.strategyLessons` добавляет первый авторский пакет
решений (first-strategy, dealer-upcard, hard-doubles) через тот же PilotLesson,
DecisionLessonSession и DecisionScene. `playableLessons` включает все три пакета;
внутренние исторические имена Pilot не означают, что новый контент — пилотные
измерения. `/lesson/first-strategy` сохраняет прежний URL; Learn также показывает
группу стратегии с переходами к трём урокам. Новый отдельный движок/вид прогресса
не добавлен. Hard 12, его signature, награды и банк checkpoint не меняются.
Дополнительные уроки не расширяют автоматически старую статистику шести узлов
в Progress; этот раздел остаётся совместимым с прежним каталогом.

Итерация 23 расширяет strategyLessons до шести уроков: hard-13-16, soft-13-17
и pairs-core используют тот же runtime. У decision разрешён `afterSplit: true`
с обязательным локализованным prompt и без Surrender. Первая карта такой руки
считается оставшейся от пары; split aces и завершённые 21 не задаются как
действующие руки. Поле включается в signature только при true, поэтому старые
decision-сессии остаются совместимы. Показ фиксированного последствия действия
по-прежнему ограничен actionMeaning и не включается наличием prompt.
...
По решению владельца итерация 10 временно отложена и технические итерации
продолжаются. Итерация 12 формализует уже существующий контент без расширения
числа уроков или заявления об успешном пользовательском пилоте.
