...
- Эта fixture проверяет предпочтительные действия при полном наборе доступных действий. Итерация 2 исправляет найденный при аудите 2026-09-05 fallback soft 18 против 3–6 без Double: Stand вместо Hit. Добавлены 16 reference-сценариев с/без Double и регрессия через TableViewModel после третьей карты. Остальные ограничения действий требуют матрицы итерации 3.
